...
namespace Clients.Data
{
    public class Order_Dal
    {
        ...
        private static async Task DeleteOrderProductsById(int Id)
        {
            List<OrderProduct> orderProductsToBeDeleted = (List<OrderProduct>)await OrderProduct_Dal.GetAll(Id);
            foreach (OrderProduct orderProduct in orderProductsToBeDeleted)
            {
                await OrderProduct_Dal.Delete(orderProduct.Order.Id);
            }
        }
    }
}
