...
namespace Clients.Blazor.Services
{
    public class OrderService
    {
        ...
        public async Task<bool> Create(Order curOrder)
        {
        ...
        public async Task<bool> Update(Order curOrder)
        {
            var content = new StringContent(JsonSerializer.Serialize(curOrder),
            Encoding.UTF8, "application/json");
        ...
        public async Task<List<OrderProduct>?> ReadOrderProducts(int OrderId)
        {
            var response = await _httpClient.GetAsync($"{URL}/OrderProducts/{OrderId}");
            if (!response.IsSuccessStatusCode)
            {
                var errorContent = await response.Content.ReadAsStringAsync();
        ...
        public async Task<bool> Update(Order curOrder, List<OrderProduct> curOrderProducts)
        {
            ...
            //List<OrderProduct> curOrderProducts = new List<OrderProduct>();
            //foreach (Product curProduct in curProducts)
            //    curOrderProducts.Add(new OrderProduct(curOrder, curProduct, 1, 1));
            var content = new StringContent(JsonSerializer.Serialize(curOrderProducts),
                     Encoding.UTF8, "application/json");
